...
#do tests of separation (for missing links)
sep.sarlm.sem.mammals.wDR.elevation.temp <- dSep(sarlm.sem.mammals.wDR.elevation.temp)
sep.sarlm.sem.birds.wDR.elevation.temp <- dSep(sarlm.sem.birds.wDR.elevation.temp)
#tests of separation show that two paths are missing:
#1) present_temperature ~ change_elevation
#2) change_temperature ~ present_elevation
#2) is non-sensical (it would make the graph not a DAG - with bidirectional arrows) and present conditions influencing the past
#so 2) is not implemented
#from here on, sems include present_temperature ~ change_elevation
#sem with sarlm for mammals wDR, elevation, temperature and change elevation and change temperature
...
grViz(sarlm.sem.mammals.geomean.wlambda.avg.elevation.temp.grViz) %>%
  export_svg %>% charToRaw %>% rsvg_pdf("./plots/sem_mammals_geolambdaavg_all_main.pdf")
...
grViz(sarlm.sem.birds.geomean.wlambda.avg.elevation.temp.grViz) %>%
  export_svg %>% charToRaw %>% rsvg_pdf("./plots/sem_birds_geolambdaavg_all_main.pdf")
